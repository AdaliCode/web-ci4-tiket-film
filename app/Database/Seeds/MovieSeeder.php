...
class MovieSeeder extends Seeder
{
    ...
    {
        $data = [];
        function getMovie($title, $cover = 'defaultCover.jpg', $release = '2024/7/10', $hour_duration = 2, $minutes_duration = 59, $trailer = 'LtNYaH61dXY')
        {
            $slug = strtolower(url_title($title, '-'));
            return ['title' => $title, 'slug' => $slug, 'cover' => $cover, 'release' => $release, 'hour_duration' => $hour_duration, 'minutes_duration' => $minutes_duration, 'trailer' => $trailer, 'created_at' => Time::now(), 'updated_at' => Time::now()];
        }
        // total film 21
